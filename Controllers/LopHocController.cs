...
namespace QuanLyTrungTam.Controllers
{
    ...
    {
        private readonly ILopHocDAL _dal;
        public LopHocController(ILopHocDAL dal) { _dal = dal; }

        [HttpGet]
        public async Task<IActionResult> GetAll()
        {
            try { return Ok(await _dal.GetAllWithKhoaHocAsync()); }
        ...
        {
            try { await _dal.AddAsync(lopHoc); return Ok(new { message = "Thêm thành công!" }); }
        ...
        {
            try { await _dal.UpdateAsync(lopHoc); return Ok(new { message = "Cập nhật thành công!" }); }
            catch (Exception ex) { return BadRequest(new { message = ex.Message }); }
        }
